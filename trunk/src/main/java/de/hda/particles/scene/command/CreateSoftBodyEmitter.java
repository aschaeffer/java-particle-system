...
public class CreateSoftBodyEmitter implements Command {

	private final Logger logger = LoggerFactory.getLogger(CreateSoftBodyEmitter.class);
	...
	public DemoHandle execute(DemoContext context, CommandConfiguration configuration, Integer transitionIterations) {
		...
		while (iterator.hasNext()) {
			...
			PooledSoftBodyEmitter emitter = new PooledSoftBodyEmitter();
			...
			context.add(emitter);
			scene.getHudManager().addCommand(new HUDCommand(HUDCommandTypes.NOTICE, "Added Soft Body Emitter"));
			logger.info("created Soft Body Emitter");
			scene.getParticleSystem().beginModification();
			emitter.setup();
			scene.getParticleSystem().endModification();
