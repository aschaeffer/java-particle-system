...
public class ShowRenderer implements Command {

	private final Logger logger = LoggerFactory.getLogger(ShowRenderer.class);
	...
	public DemoHandle execute(DemoContext context, CommandConfiguration configuration, Integer transitionIterations) {
		...
		try {
			...
			while (iterator.hasNext()) {
				...
				scene.getRendererManager().setVisibility(clazz, true);
				scene.getParticleSystem().endModification();
				// scene.endModification();
			}
		} catch (Exception e) {
			logger.error("could not show renderer of type " + type);
		}
		return null;
	}
